                ...
                for t in tokens:
                    if len(t) > 0 and t in index_dict:
                        ...
                        for prev_token in last_n:
                            col_indexes.add(index_dict[prev_token])
                        col_indexes_sort = list(col_indexes)
                        ll = main_matrix[index_dict[t] , col_indexes_sort]
                        for i in range(len(ll)):
                            ll[i] = ll[i] + 1
                        main_matrix[index_dict[t] , col_indexes_sort] = ll
                        last_n.append(t)
...
def ppmi_inplace(matrix, k=5):
    ...
    file_name = tmp_base + '-ppmi-' + str(k) + '.txt'
    if os.path.isfile(file_name):
        print('Loading ppmi from file ')
        load_lil_from_file(file_name, matrix)
    ...
    save_lil_to_file(file_name, matrix)
...
def build_vocab_index_from_corpus(corpus_input_file, high_pass, low_pass):
    ...
    for v, freq in vocab_freq.items():
        add = True
        if high_pass is not None and freq > high_pass:
            add = False
        if low_pass is not None and freq < low_pass:
            add = False
        if add:
            lst.append(v)
...
def build_model(corpus_file, index_file, matrix_file, min_freq, window_size):
    ...
    #matrix = sparse.lil_matrix((len(index_dict), len(index_dict)))
    ppmi_inplace(matrix , k=0)
    print('shape matrix is ')
    print(matrix.shape)
    u, s, vt = calculate_svd(matrix)
    np.savetxt(matrix_file, u * s * s)
...
def test_model(index_file, matrix_file):
    ...
    for t in test_lst:
        if t in index_dict:
            print(t)
            vec = embedding_matrix[index_dict[t]][:]
            print_closest(vec, embedding_matrix, rev_index, top_n)
